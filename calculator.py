# impoort tkinter package
# Tk GUI toolkit에 대한 표준 파이썬 인터페이스
# 명령줄에서 python -m tkinter를 실행하여 설치된 Tcl/Tk 버전 확인 가능
# 또는 import tkinter  / tkinter._test()를 연이어 실행하여 봄
import tkinter as tk

## window 생성
winCalc = tk.Tk()
winCalc.title("Simple Calculator by KITA")
winCalc.geometry("350x500")

## frame 생성
# 계산결과를 보여줄 공간
upper_frame = tk.Frame(winCalc, width=400, height=70)
upper_frame.pack(pady=40)

# 계산기 버튼들이 배치될 공간
down_frame = tk.Frame(winCalc, width=400, height=100)
down_frame.pack(padx=10, pady=10)
## 계산 결과를 출력할 텍스트엔트리 upper_frame에 추가 정의
...
btn_mul = tk.Button(down_frame, text='×', padx=15, pady=10, font=("Courier", 15), command=button_mul, bg='orange')
